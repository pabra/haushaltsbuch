...
def get_order_by_from_request(req):
    ...
    for order in req.args.getlist('orderBy'):
        ...
        if not re.match(r'^[a-zA-Z0-9_]+$', order_tuple[0]):
            raise ValueError('Order field must match regex "^[a-zA-Z0-9_]+$": %r' % (order_tuple[0],))
...
def get_filter_from_request(req):
    ...
    for flt in req.args.getlist('filter'):
        ...
        if not re.match(r'^[a-zA-Z0-9_]+$', filter_tuple[0]):
            raise ValueError('Filter field must match regex "^[a-zA-Z0-9_]+$": %r' % (filter_tuple[0],))

        if not filter_tuple[1] in compare_symbols:
            raise ValueError('Filter must use allowed compare symbol %r: %r' % (compare_symbols, filter_tuple[1]))

        if not filter_tuple[0] in filter_parsers:
            raise ValueError('No parser defined to filter field %r.' % (filter_tuple[0],))
