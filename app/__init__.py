...
def category_index():
    updates_available, _ = database.kv_get("updates_available", "int")
    return render_template(
        "category.html", page="category", updates_available=updates_available
    )
...
def category_update():
    d = {"method": request.method, "form": request.json, "action": "update"}
...
def category_add():
    d = {"method": request.method, "form": request.json, "action": "add"}
...
def category_delete(category_id):
    d = {"method": request.method, "category_id": category_id, "action": "delete"}
...
def expense_get_all():
    ...
    return jsonify(
        {
            "data": database.date_to_json(
                database.get_all_expenses(order_by=order_by_list, filter_by=filter_list)
            )
        }
    )
...
def expense_delete(expense_id):
    d = {"method": request.method, "expense_id": expense_id, "action": "delete"}
...
def expense_notes(category_id, txt):
    ...
    return jsonify({"OK": True, "status": 200, "notes": notes})
