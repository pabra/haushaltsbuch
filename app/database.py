...
def connect_db():
    from . import app
    conn = sqlite3.connect(app.config['DATABASE'], detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES)
...
def insert_rand():
    ...
    with closing(connect_db()) as db:
        ...
        colors = ('#e06666', '#f6b26b', '#ffd966', '#b6d7a8', '#76a5af', '#6fa8dc', '#8e7cc3', '#c27ba0')
        i = 1
        inserted = 0
        ...
        while i < 1000:
            data = {
                'date': datetime.date.today() - datetime.timedelta(days=random.randint(1, 500)),
...
def get_category_by_id(category_id):
    assert isinstance(category_id, int), translate('category_id must be integer not %r') % type(category_id)
...
def delete_category(category_id):
    ...
    assert not exp, translate('cannot delete category %(category_name)r with %(nr_of_expenses)s expenses assigned') % {'category_name': cat['name'], 'nr_of_expenses': len(exp)}
...
def add_category(data):
    ...
    assert isinstance(color, str), translate('color must be string not %r') % type(color)
    assert is_valid_color(color), translate('value of color does not look valid: %r') % color
...
def update_category(data):
    ...
    if 'name' in data:
        ...
        assert not dup_cat, translate('category already exists %(existing)r, %(duplicate)r') % {'existing': cat, 'duplicate': dup_cat}
    ...
    if q_mid:
        ...
        return {'rows': res,
                'data': get_category_by_id(category_id)}


# manage expenses
def get_all_expenses(order_by=[('date', 'asc')], filter_by=[]):
    q_args = []
    order_by = [('category_id', order[1]) if order[0] == 'category' else order
                for order in order_by]

    order_by_str = ', '.join(['%s %s' % order
                              for order in order_by])
    ...
    filter_by_str = ' AND '.join(['%s %s ?' % flt[:2]
                                  for flt in filter_by])
...
def get_expense_by_id(expense_id):
    assert isinstance(expense_id, int), translate('expense_id must be integer not %r') % type(expense_id)
...
def update_expense(data):
    ...
    if 'date' in data:
        date = data['date']
        assert isinstance(date, datetime.date), translate('date must be datetime.date not %r') % type(date)
    ...
    if 'note' in data:
        note = data['note']
        assert isinstance(note, (str, type(None))), 'note must be string or None not %r' % type(note)
...
def get_summary(year=None):
    ...
    q = '''
        ...
        ORDER BY date ASC
    ''' % (', '.join(('SUM(CASE category_id WHEN %(id)s THEN value ELSE 0 END) AS value_%(id)s' % cat for cat in categories)),
           ', '.join(('SUM(CASE category_id WHEN %(id)s THEN 1 ELSE 0 END) AS count_%(id)s' % cat for cat in categories)),
